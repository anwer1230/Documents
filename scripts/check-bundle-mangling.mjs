...
if (!fs.existsSync(dir)) {
  console.log(`[check-bundle-mangling] Directory ${dir} does not exist yet; skipping check.`);
  process.exit(0);
}

function findChunks(d, rel = '') {
  let res = [];
  ...
  for(const entry of entries) {
    ...
    if(entry.isDirectory()) {
      res = res.concat(findChunks(fullPath, relPath));
    } else if(entry.isFile() && entry.name.endsWith('.js') && fs.existsSync(fullPath + '.map')) {
      res.push(relPath);
    }
  }
  return res;
