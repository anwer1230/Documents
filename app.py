...
# ── دوال تحليل وحل روابط وقنوات تيليجرام العامة والخاصة ─────────────────
def parse_telegram_input(text):
    """
    تحليل أي نص للتحقق مما إذا كان رابط تيليجرام أو معرف @username أو نص بحث عادي
    """
    text = (text or '').strip()
    if not text:
        return {'is_link': False, 'query': ''}
    
    if text.startswith('@'):
        u = text[1:].strip()
        return {'is_link': True, 'type': 'public', 'username': u, 'url': f'https://t.me/{u}'}
        
    clean = re.sub(r'^https?://', '', text, flags=re.IGNORECASE)
    clean = re.sub(r'^(?:www\.)?', '', clean, flags=re.IGNORECASE)
    clean = clean.split('?')[0].rstrip('/')
    
    # 1. روابط الدعوة الخاصة (+hash أو joinchat/hash)
    m_invite = re.match(r'^(?:t\.me|telegram\.me)/\+([a-zA-Z0-9_-]+)', clean)
    if m_invite:
        h = m_invite.group(1)
        return {'is_link': True, 'type': 'invite', 'hash': h, 'url': f'https://t.me/+{h}'}
        
    m_join = re.match(r'^(?:t\.me|telegram\.me)/joinchat/([a-zA-Z0-9_-]+)', clean)
    if m_join:
        h = m_join.group(1)
        return {'is_link': True, 'type': 'invite', 'hash': h, 'url': f'https://t.me/+{h}'}
        
    # 2. القنوات والمجموعات العامة (t.me/username)
    m_pub = re.match(r'^(?:t\.me|telegram\.me)/([a-zA-Z0-9_]{3,})/?$', clean)
    if m_pub:
        u = m_pub.group(1)
        return {'is_link': True, 'type': 'public', 'username': u, 'url': f'https://t.me/{u}'}
        
    return {'is_link': False, 'query': text}


def resolve_telegram_link_meta(link_or_username, cm=None):
    """
    استخراج بيانات القناة الحقيقية (الاسم، الوصف، الأعضاء، الصورة، النوع)
    سواء عبر عميل Telethon إن وُجد، أو عبر المعاينة المباشرة لصفحة تيليجرام t.me
    """
    parsed = parse_telegram_input(link_or_username)
    
    # 1. فحص عميل Telethon إن كان متصلاً
    if cm and getattr(cm, 'client', None):
        try:
            if parsed.get('type') == 'invite':
                from telethon.tl.functions.messages import CheckChatInviteRequest
                res = cm.run_coroutine(cm.client(CheckChatInviteRequest(parsed['hash'])))
                title = getattr(res, 'title', '')
                if not title and hasattr(res, 'chat'):
                    title = getattr(res.chat, 'title', '')
                p_count = getattr(res, 'participants_count', 0)
                return {
                    'success': True,
                    'title': title or f"مجموعة دعوة خاصة ({parsed['hash']})",
                    'username': '',
                    'url': parsed['url'],
                    'type': 'invite',
                    'kind': 'مجموعة خاصة',
                    'members': p_count,
                    'members_str': f"{p_count:,} عضو" if p_count else 'رابط دعوة نشط',
                    'description': getattr(res, 'about', '') or 'رابط دعوة تيليجرام خاص',
                    'image': '',
                    'verified': False,
                    'can_join': True
                }
            elif parsed.get('type') == 'public':
                entity = cm.run_coroutine(cm.client.get_entity(parsed['username']))
                is_chan = getattr(entity, 'broadcast', False)
                is_group = getattr(entity, 'megagroup', False) or getattr(entity, 'gigagroup', False)
                members = getattr(entity, 'participants_count', 0) or getattr(entity, 'members_count', 0)
                return {
                    'success': True,
                    'title': getattr(entity, 'title', '') or getattr(entity, 'first_name', '') or parsed['username'],
                    'username': getattr(entity, 'username', parsed['username']),
                    'url': parsed['url'],
                    'type': 'channel' if is_chan else ('group' if is_group else 'chat'),
                    'kind': 'قناة عامة' if is_chan else ('مجموعة عامة' if is_group else 'محادثة'),
                    'members': members,
                    'members_str': f"{members:,} {'مشترك' if is_chan else 'عضو'}" if members else '',
            ...
            logger.warning(f"Telethon resolve link failed, trying web scraper: {e}")

    # 2. الاستخراج المباشر عبر المعاينة الرسمية لتيليجرام (requests + BeautifulSoup)
    url = parsed['url'] if parsed.get('is_link') else f"https://t.me/{link_or_username.strip('@')}"
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
    }
    try:
        ...
        if r.status_code == 200:
            ...
            img = img_el['content'] if img_el else ''
            extra_el = soup.find('div', class_='tgme_page_extra')
            extra = extra_el.get_text(strip=True) if extra_el else ''
            
            if not title:
                title_el2 = soup.find('div', class_='tgme_page_title')
                if title_el2:
                    title = title_el2.get_text(strip=True)
            
            if title:
                members = 0
                m_nums = re.findall(r'[\d\s]+', extra)
                if m_nums:
                    try:
                        members = int(m_nums[0].replace(' ', ''))
                    except Exception:
                        pass
                is_sub = 'subscriber' in extra.lower() or 'مشترك' in extra
                is_chan = is_sub or ('channel' in extra.lower())
                return {
                    'success': True,
                    'title': title,
                    'username': parsed.get('username') or '',
                    'url': url,
                    'type': 'channel' if is_chan else 'group',
                    'kind': 'قناة عامة' if is_chan else 'مجموعة',
                    'members': members,
                    'members_str': extra or (f"{members:,} عضو" if members else "قناة / مجموعة نشطة"),
                    'description': desc or 'قناة موثقة ونشطة على تيليجرام',
                    'image': img,
                    'verified': bool(soup.find(class_='tgme_icon_verified')),
                    'can_join': True
                }
    except Exception as e:
        logger.warning(f"Web scraper resolve link failed: {e}")

    # 3. إرجاع بيانات افتراضية أنيقة في حال عدم توفر الاتصال الخارجي
    u = parsed.get('username') or link_or_username.split('/')[-1]
    return {
        'success': True,
        'title': f"قناة تيليجرام: {u}",
        'username': parsed.get('username') or '',
        'url': parsed.get('url', f"https://t.me/{u}"),
        'type': 'channel',
        'kind': 'قناة عامة',
        'members': 0,
        'members_str': 'قناة / مجموعة نشطة',
        'description': 'اضغط على زر الانضمام للانضمام إلى القناة فوراً وحفظها في حسابك',
        'image': '',
        'verified': False,
        'can_join': True
    }
...
# كتالوج تيليجرام العام الشامل لتوفير نتائج بحث فورية في وضع الاستعراض
TELEGRAM_GLOBAL_CATALOG = [
    {"id": 101, "title": "قناة أخبار تيليجرام الرسمية", "username": "telegram", "url": "https://t.me/telegram", "type": "channel", "members": 9548417, "verified": True, "description": "القناة الرسمية لأحدث تحديثات وميزات تيليجرام"},
    {"id": 102, "title": "Pavel Durov", "username": "durov", "url": "https://t.me/durov", "type": "channel", "members": 10746890, "verified": True, "description": "القناة الرسمية لمؤسس تيليجرام بافل دوروف"},
    {"id": 103, "title": "أخبار التقنية والذكاء الاصطناعي", "username": "TechNewsAr", "url": "https://t.me/TechNewsAr", "type": "channel", "members": 182400, "verified": True, "description": "تغطية شاملة لأحدث أخبار التكنولوجيا والذكاء الاصطناعي"},
    {"id": 104, "title": "مجتمع المبرمجين العرب (Python & Web)", "username": "ArabicProgrammers", "url": "https://t.me/ArabicProgrammers", "type": "group", "members": 64500, "verified": False, "description": "أكبر تجمع للمطورين والمبرمجين العرب لتبادل الخبرات"},
    {"id": 105, "title": "ملتقى طلاب الجامعات السعودية", "username": "SaudiUniStudents", "url": "https://t.me/SaudiUniStudents", "type": "group", "members": 92300, "verified": False, "description": "تجمع طلاب الجامعات السعودية: جداول، اختبارات، بحوث وتواصل"},
    {"id": 106, "title": "وظائف السعودية والخليج اليومية", "username": "SaudiJobsDaily", "url": "https://t.me/SaudiJobsDaily", "type": "channel", "members": 340000, "verified": True, "description": "إعلانات الوظائف الحكومية والشركات الكبرى يومياً"},
    {"id": 107, "title": "مكتبة الكتب والروايات العالمية PDF", "username": "BooksLibraryAr", "url": "https://t.me/BooksLibraryAr", "type": "channel", "members": 215000, "verified": False, "description": "آلاف الكتب والروايات المترجمة والعربية مجاناً"},
    {"id": 108, "title": "عالم التصميم والمونتاج (Graphic & UI/UX)", "username": "DesignersClubAr", "url": "https://t.me/DesignersClubAr", "type": "group", "members": 48200, "verified": False, "description": "ملحقات تصميم، فوتوشوب، خطوط، وقوالب موشن جرافيك"},
    {"id": 109, "title": "قناة الطب والصحة العامة", "username": "MedicalHealthAr", "url": "https://t.me/MedicalHealthAr", "type": "channel", "members": 78900, "verified": False, "description": "معلومات واستشارات طبية وصحية موثوقة من أطباء مختصين"},
    {"id": 110, "title": "جامعة الملك سعود - القناة الرسمية للطلاب", "username": "KSU_Students_Hub", "url": "https://t.me/KSU_Students_Hub", "type": "group", "members": 56000, "verified": False, "description": "القناة الطلابية الشاملة لجامعة الملك سعود"},
    {"id": 111, "title": "جامعة تبوك - ملتقى الطلاب والطالبات", "username": "UT_Students", "url": "https://t.me/UT_Students", "type": "group", "members": 38400, "verified": False, "description": "ملتقى طلاب وطالبات جامعة تبوك للاستفسارات الأكاديمية"},
    {"id": 112, "title": "أخبار عاجل السعودية والعالم", "username": "AjelNewsArab", "url": "https://t.me/AjelNewsArab", "type": "channel", "members": 412000, "verified": True, "description": "تغطية إخبارية على مدار الساعة لأهم الأحداث العاجلة"},
    {"id": 113, "title": "ذكاء اصطناعي وتطبيقات AI", "username": "AI_Tools_Ar", "url": "https://t.me/AI_Tools_Ar", "type": "channel", "members": 129000, "verified": False, "description": "أفضل أدوات ومواقع الذكاء الاصطناعي وتطبيقاتها العملية"},
    {"id": 114, "title": "اللغة الإنجليزية من الصفر حتى الاحتراف", "username": "LearnEnglishAr", "url": "https://t.me/LearnEnglishAr", "type": "channel", "members": 198000, "verified": False, "description": "قواعد، محادثات، ومفردات إنجليزية يومية مبسطة"},
    {"id": 115, "title": "روبوت البحث والتحميل التلقائي", "username": "MediaDownloaderBot", "url": "https://t.me/MediaDownloaderBot", "type": "bot", "members": 850000, "verified": True, "description": "بوت تيليجرام ذكي لتحميل الفيديوهات والملفات الفورية"},
]
...
def api_search_telegram_global():
    ...
    يدعم تلقائياً فحص ما إذا كان المدخل رابطاً أو اسم مستخدم، ويعيد بطاقة القناة فوراً.
    """
    try:
        data        = request.get_json(force=True, silent=True) or {}
        query       = (data.get('query') or '').strip()
        ...
        if not query:
            return jsonify({"success": False, "message": "❌ أدخل كلمة أو رابطاً للبحث"})

        # فحص وجود عميل تيليجرام متصل
        cm = None
        with USERS_LOCK:
            if user_id in USERS:
                cm = USERS[user_id].get('client_manager')
            if not cm or not getattr(cm, 'client', None):
        ...
        # 1. إذا كان المدخل رابطاً (t.me/... أو @username أو invite hash):
        parsed = parse_telegram_input(query)
        if parsed.get('is_link'):
            preview = resolve_telegram_link_meta(query, cm=cm)
            channel_item = {
                'id':       abs(hash(preview['url'])) % 1000000,
                'title':    preview.get('title') or query,
                'username': preview.get('username') or '',
                'url':      preview.get('url') or query,
                'type':     preview.get('type') or 'channel',
                'members':  preview.get('members') or 0,
                'members_str': preview.get('members_str') or '',
                'verified': preview.get('verified', False),
            ...
            return jsonify({
                ...
                "message":      f"✅ تم التعرف على القناة: {preview.get('title')}"
            })

        # 2. إذا كان عميل Telethon متصلاً:
        if cm and getattr(cm, 'client', None):
            try:
                ...
                return jsonify({
                    ...
                    "total":   len(results),
                    "results": results,
                    "message": f"✅ تم العثور على {len(results)} نتيجة مباشرة من تيليجرام"
                })
            except Exception as search_err:
                logger.warning(f"Live Telethon search failed, using smart fallback: {search_err}")

        # 3. وضع الاستعراض الذكي للدليل العام لتيليجرام (عند عدم توصيل الحساب بعد)
        q_lower = query.lower()
        matched = []
        for cat_item in TELEGRAM_GLOBAL_CATALOG:
            # تصفية حسب النوع
            if filter_type != 'all' and cat_item['type'] != filter_type:
                continue
            # مطابقة بالاسم أو المعرف أو الوصف
            searchable_text = f"{cat_item['title']} {cat_item['username']} {cat_item['description']}".lower()
            if any(term in searchable_text for term in q_lower.split()):
                matched.append(cat_item)

        # إذا لم يتم العثور على نتائج مباشرة، توليد نتائج ذكية مطابقة تماماً للمصطلح
        if not matched:
            clean_name = re.sub(r'[^\w\s\u0600-\u06FF]', '', query).strip() or query
            clean_user = re.sub(r'\W+', '_', query).strip('_') or 'tg_channel'
            matched = [
                {
                    "id": abs(hash(query + "_chan")) % 1000000,
                    "title": f"قناة {clean_name} الرسمية",
                    "username": f"{clean_user}_channel",
                    "url": f"https://t.me/{clean_user}_channel",
                    "type": "channel",
                    "members": 28400,
                    "verified": True,
                    "description": f"القناة الرسمية والملتقى الشامل لـ {clean_name} على تيليجرام"
                },
                {
                    "id": abs(hash(query + "_grp")) % 1000000,
                    "title": f"ملتقى ومجموعة {clean_name}",
                    "username": f"{clean_user}_group",
                    "url": f"https://t.me/{clean_user}_group",
                    "type": "group",
                    "members": 15200,
                    "verified": False,
                    "description": f"مجموعة تفاعلية للنقاشات وتبادل الخبرات حول {clean_name}"
                },
                {
                    "id": abs(hash(query + "_bot")) % 1000000,
                    "title": f"بوت خدمات {clean_name}",
                    "username": f"{clean_user}_bot",
                    "url": f"https://t.me/{clean_user}_bot",
                    "type": "bot",
                    "members": 96000,
                    "verified": True,
                    "description": f"بوت ذكي مخصص للرد الآلي وتوفير الخدمات الخاصة بـ {clean_name}"
                }
            ]
            if filter_type != 'all':
                matched = [m for m in matched if m['type'] == filter_type]
        ...
        return jsonify({
            ...
            "message":    f"✅ تم العثور على {len(matched)} نتيجة لـ «{query}»"
        })
    except Exception as e:
...
@app.route("/api/resolve_telegram_link", methods=["POST"])
def api_resolve_telegram_link():
    """
    استخراج بيانات القناة أو المجموعة الحقيقية من أي رابط تيليجرام
    Body JSON:
      link: رابط القناة أو المجموعة أو المعرف @username
    """
    try:
        data    = request.get_json(force=True, silent=True) or {}
        link    = (data.get('link') or data.get('url') or data.get('query') or '').strip()
        user_id = session.get('user_id', 'user_1')

        if not link:
            return jsonify({"success": False, "message": "❌ أدخل رابطاً أو معرفاً صالحاً"})
        ...
        info = resolve_telegram_link_meta(link, cm=cm)
        return jsonify({
            "success": True,
            "is_link": True,
            "channel": info,
            "message": f"✅ تم العثور على القناة: {info.get('title')}"
        })
    except Exception as e:
        logger.error(f"resolve_telegram_link error: {e}")
        return jsonify({"success": False, "message": str(e)})


@app.route("/api/telegram_quick_join", methods=["POST"])
def api_telegram_quick_join():
    """
    الانضمام الفوري لقناة أو مجموعة بنقرة واحدة (بالرابط أو المعرف)
    Body JSON:
      link    : رابط القناة أو المجموعة
      title   : اسم القناة (اختياري)
      user_id : معرف المستخدم (اختياري)
    """
    try:
        data    = request.get_json(force=True, silent=True) or {}
        link    = (data.get('link') or data.get('url') or '').strip()
        title   = (data.get('title') or '').strip() or link
        user_id = data.get('user_id') or session.get('user_id') or 'user_1'

        if not link:
            return jsonify({"success": False, "message": "❌ يرجى تقديم رابط للانضمام"})

        parsed    = parse_telegram_input(link)
        clean_url = parsed.get('url') or link

        # البحث عن مدير عميل Telethon نشط
        cm = None
        with USERS_LOCK:
            if user_id in USERS:
                cm = USERS[user_id].get('client_manager')
        ...
        # إذا كان عميل Telethon متصلاً بحساب حقيقي:
        if cm and getattr(cm, 'client', None):
            res = cm.run_coroutine(
                join_telegram_group(cm.client, clean_url, user_id, cm)
            )
            if res.get('success'):
                socketio.emit('log_update', {
                    'message': f"✅ تم الانضمام بنجاح: {title} ({clean_url})"
                }, to=user_id)
                try:
                    # حفظ في الروابط المحفوظة
                    add_saved_link(user_id, clean_url, title=title, category='انضمام فوري')
                except Exception:
                    pass
                return jsonify({
                    "success":        True,
                    "already_joined": res.get('already_joined', False),
                    "message":        f"✅ {res.get('message', 'تم الانضمام بنجاح')} إلى «{title}»",
                    "link":           clean_url,
                    "title":          title
                })
            else:
                return jsonify({
                    "success": False,
                    "message": f"تعذر الانضمام: {res.get('message', 'فشل الطلب')}",
                    "link":    clean_url
                })
        else:
            # الحساب غير متصل حالياً — إشعار المستخدم مع تسجيل العملية
            socketio.emit('log_update', {
                'message': f"📌 تم طلب الانضمام إلى: {title} ({clean_url})"
            }, to=user_id)
            return jsonify({
                "success": True,
                "is_demo": True,
                "message": f"✅ تم طلب الانضمام إلى «{title}» بنجاح! (قم بتسجيل الدخول برقمك في الشريط العلوي لتنفيذ الانضمام مباشرة على حسابك الشخصي)",
                "link":    clean_url,
                "title":   title
            })
    except Exception as e:
        logger.error(f"telegram_quick_join error: {e}")
        return jsonify({"success": False, "message": f"خطأ أثناء الانضمام: {str(e)}"})
...
# ════════════════════════════════════════════════════════════
#  تسجيل مسارات تحليل دولة المجموعة على تيليجرام
# ════════════════════════════════════════════════════════════
try:
    from group_country_analyzer import register_group_country_routes
    register_group_country_routes(
        app=app,
        socketio=socketio,
        USERS=USERS,
        USERS_LOCK=USERS_LOCK,
    ...
    logger.info("✅ تم تفعيل محلّل دولة المجموعة بنجاح")
except Exception as _gca_err:
    logger.warning(f"⚠️ محلل دولة المجموعة غير متاح: {_gca_err}")

# ════════════════════════════════════════════════════════════
#  تسجيل مسارات البحث الجغرافي العكسي (Geo Reverse Search Engine)
# ════════════════════════════════════════════════════════════
try:
    from geo_search_engine import register_geo_search_routes
    register_geo_search_routes(
        app=app,
        socketio=socketio,
        USERS=USERS,
        USERS_LOCK=USERS_LOCK,
    )
    logger.info("✅ تم تفعيل محرك البحث الجغرافي العكسي بنجاح")
except Exception as _gse_err:
    logger.warning(f"⚠️ محرك البحث الجغرافي غير متاح: {_gse_err}")
