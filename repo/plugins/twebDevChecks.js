...
function getBinaryPath(rootDir, name) {
  ...
  const localBin = path.join(rootDir, 'node_modules', '.bin', filename);
...
export default function devChecks(rootDir = process.cwd()) {
  return {
    ...
    configureServer(server) {
      ...
      function run(name, args, label, onLine) {
        const binPath = getBinaryPath(rootDir, name);
        let child;
        try {
          ...
          log(label, `Skipping ${name}: not found or failed to spawn`, process.stdout);
          return null;
        }
        ...
        child.once('error', (error) => {
          log(label, `Note: ${error.message}`, process.stdout);
        });
