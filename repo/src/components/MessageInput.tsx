...
export const MessageInput: React.FC<MessageInputProps> = ({
  ...
  // Helper for Telegram WYSIWYG text entities
  const applyFormatting = (prefix: string, suffix: string = prefix) => {
    ...
    const selected = currentVal.substring(start, end) || (isAr ? 'نص' : 'text');
  ...
  const handleSend = () => {
    if (!text.trim()) return;
    onSendMessage(text.trim(), replyToMessage || undefined, undefined, {
  ...
  return (
    <div className={`p-3 relative border-t ${isDark ? 'bg-[#17212b] border-[#242f3d]' : 'bg-white border-gray-200'}`}>
      ...
      {/* Active Forum Topic Banner */}
      {selectedTopicId && chat?.topics && (
        <div
          className={`flex items-center justify-between px-3 py-1.5 mb-2 rounded-xl border text-xs ${
            isDark ? 'bg-[#242f3d]/70 border-gray-700 text-gray-200' : 'bg-blue-50/70 border-blue-200 text-gray-800'
          }`}
        >
          <div className="flex items-center gap-2">
            <Hash className="w-3.5 h-3.5 text-[#3390ec]" />
            <span className="font-semibold text-xs text-[#3390ec]">
              {chat.topics.find((t) => t.id === selectedTopicId)?.title || `Topic #${selectedTopicId}`}
            </span>
            <span className="text-[10px] text-gray-400">
              {isAr ? '(الردود تذهب لهذا الموضوع)' : '(Replies go to this topic)'}
            </span>
          </div>
          {onOpenTopicsDrawer && (
            <button
              type="button"
              onClick={onOpenTopicsDrawer}
              className="text-[11px] text-[#3390ec] hover:underline font-medium"
            >
              {isAr ? 'تغيير' : 'Change'}
            </button>
          )}
      ...
      {/* WYSIWYG Message Entities Formatting Bar */}
      {showFormattingToolbar && (
        <div
          className={`flex items-center gap-1 px-3 py-1.5 mb-2 rounded-xl border text-xs overflow-x-auto ${
            isDark ? 'bg-[#242f3d] border-gray-700/80 text-gray-200' : 'bg-gray-100 border-gray-300 text-gray-700'
          }`}
        >
          <span className="text-[10px] text-gray-400 font-medium me-1 shrink-0">
            {isAr ? 'تنسيق:' : 'Format:'}
          </span>
          <button
            type="button"
            onClick={() => applyFormatting('**')}
            className="p-1.5 rounded-lg hover:bg-[#3390ec]/20 hover:text-[#3390ec] font-bold shrink-0"
          ...
          <button
            type="button"
            onClick={() => applyFormatting('*')}
            className="p-1.5 rounded-lg hover:bg-[#3390ec]/20 hover:text-[#3390ec] italic shrink-0"
          ...
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-[#3390ec]/20 hover:text-[#3390ec] underline shrink-0"
          ...
          <button
            type="button"
            onClick={() => applyFormatting('~~')}
            className="p-1.5 rounded-lg hover:bg-[#3390ec]/20 hover:text-[#3390ec] line-through shrink-0"
          ...
          <button
            type="button"
            onClick={() => applyFormatting('`')}
            className="p-1.5 rounded-lg hover:bg-[#3390ec]/20 hover:text-[#3390ec] font-mono shrink-0"
            title={isAr ? 'كود (Monospace)' : 'Monospace'}
          >
          ...
          <button
            type="button"
            onClick={() => applyFormatting('||')}
            className="p-1.5 rounded-lg hover:bg-[#3390ec]/20 hover:text-[#3390ec] flex items-center gap-1 font-semibold text-[11px] shrink-0"
            title={isAr ? 'نص مخفي (Spoiler)' : 'Spoiler'}
          >
          ...
          <button
            type="button"
            onClick={() => applyFormatting('> ')}
            className="p-1.5 rounded-lg hover:bg-[#3390ec]/20 hover:text-[#3390ec] shrink-0"
            title={isAr ? 'اقتباس (Blockquote)' : 'Blockquote'}
          >
          ...
          <>
            ...
            <button
              ...
              className={`p-2 rounded-full transition ${
                showFormattingToolbar
                  ? 'text-[#3390ec] bg-[#3390ec]/10'
                  : isDark
                  ? 'text-gray-400 hover:text-white hover:bg-[#202b36]'
                  : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'
              }`}
              title={isAr ? 'تنسيق النص (Spoiler / Bold / Quotes)' : 'Text formatting'}
            >
              <span className="font-serif font-bold text-sm leading-none">T</span>
            </button>

            {/* Forum Topic Switcher Button */}
            {chat?.isForum && onOpenTopicsDrawer && (
              <button
                type="button"
                onClick={onOpenTopicsDrawer}
            ...
            {/* Telegram Stars Quick Action Button */}
            {onOpenStarsModal && (
              <button
                type="button"
                onClick={onOpenStarsModal}
                className="p-2 rounded-full text-amber-400 hover:text-amber-300 hover:bg-amber-500/10 transition shrink-0"
            ...
            {/* Schedule Message Quick Action Button */}
            {onOpenScheduleModal && (
              <button
                type="button"
                ...
                title={isAr ? 'جدولة رسالة' : 'Schedule message'}
              >
