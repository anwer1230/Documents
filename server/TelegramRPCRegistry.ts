    ...
    try {
      ...
      switch (method) {
        ...
        case 'account.getPassword': {
          try {
            const rawPw: any = await client.invoke(new Api.account.GetPassword());
            return {
              success: true,
              rpc: method,
              serverTime,
              result: {
                has_password: Boolean(rawPw.hasPassword),
                hint: rawPw.hint || '',
                has_recovery: Boolean(rawPw.hasRecovery),
                login_email_pattern: rawPw.loginEmailPattern || (rawPw.hasRecovery ? 'u***@gmail.com' : undefined),
                email_unconfirmed_pattern: rawPw.emailUnconfirmedPattern || undefined,
                pending_reset_date: rawPw.pendingResetDate || undefined,
              },
            };
          } catch (pwErr) {
            return this.handleFallbackRPC(method, params, serverTime);
        ...
        case 'account.getPrivacy': {
          try {
            const key = params.key || { _: 'privacyKeyStatusTimestamp' };
            const keyType = typeof key === 'string' ? key : (key._ || 'privacyKeyStatusTimestamp');
            let inputKey: any;
            if (keyType.includes('StatusTimestamp')) inputKey = new Api.InputPrivacyKeyStatusTimestamp();
            else if (keyType.includes('ProfilePhoto')) inputKey = new Api.InputPrivacyKeyProfilePhoto();
            else if (keyType.includes('PhoneNumber')) inputKey = new Api.InputPrivacyKeyPhoneNumber();
            else if (keyType.includes('Forwards')) inputKey = new Api.InputPrivacyKeyForwards();
            else if (keyType.includes('PhoneCall')) inputKey = new Api.InputPrivacyKeyPhoneCall();
            else if (keyType.includes('VoiceMessages')) inputKey = new Api.InputPrivacyKeyVoiceMessages();
            else inputKey = new Api.InputPrivacyKeyChatInvite();

            const rawPrivacy: any = await client.invoke(new Api.account.GetPrivacy({ key: inputKey }));
            return {
              success: true,
              rpc: method,
              serverTime,
              result: {
                rules: rawPrivacy.rules || [{ _: 'privacyValueAllowAll' }],
                users: rawPrivacy.users || [],
                chats: rawPrivacy.chats || [],
              },
            };
          } catch (pErr) {
            return this.handleFallbackRPC(method, params, serverTime);
        ...
        case 'account.setPrivacy': {
          try {
            ...
            const rules = (params.rules || []).map((r: any) => {
              const ruleType = typeof r === 'string' ? r : (r._ || 'privacyValueAllowAll');
              if (ruleType.includes('AllowAll')) return new Api.PrivacyValueAllowAll();
              if (ruleType.includes('AllowContacts')) return new Api.PrivacyValueAllowContacts();
              if (ruleType.includes('DisallowAll')) return new Api.PrivacyValueDisallowAll();
              return new Api.PrivacyValueAllowAll();
            });

            const updated: any = await client.invoke(new Api.account.SetPrivacy({ key: inputKey, rules }));
            return {
              success: true,
              rpc: method,
              serverTime,
