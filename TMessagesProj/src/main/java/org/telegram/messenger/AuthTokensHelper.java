...
public class AuthTokensHelper {
    ...
    /**
     * Goal 3: Guarantees user session keys and credentials remain protected across app restarts.
     */
    public static void protectRealUserSession(int account) {
        UserConfig userConfig = UserConfig.getInstance(account);
        if (userConfig.isClientActivated() && userConfig.getCurrentUser() != null) {
            userConfig.saveConfig(true);
            SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("tg_auth_tokens", Context.MODE_PRIVATE);
            preferences.edit()
                .putBoolean("is_session_protected_" + account, true)
                .putLong("protected_user_id_" + account, userConfig.clientUserId)
                .apply();
        }
    }

    /**
     * Goal 3: Registers device push token and marks user as registered for push notifications.
     */
    public static void registerDeviceWithPushToken(int account) {
        UserConfig userConfig = UserConfig.getInstance(account);
        userConfig.registeredForPush = true;
        userConfig.saveConfig(false);
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("Notifications", Context.MODE_PRIVATE);
        preferences.edit()
            .putBoolean("push_registered_" + account, true)
            .apply();
    }
