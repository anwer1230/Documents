...
async function startServer() {
  ...
  // Static embedded credentials
  const STATIC_API_ID = 22043994;
  const STATIC_API_HASH = '56f64582b363d367280db96586b97801';

  // Helper to create a new connected Telegram MTProto client
  const createNewTelegramClient = async (numericApiId: number = STATIC_API_ID, stringApiHash: string = STATIC_API_HASH): Promise<TelegramClient> => {
    const stringSession = new sessions.StringSession('');
    const effectiveApiId = Number(numericApiId) || STATIC_API_ID;
    const effectiveApiHash = stringApiHash || STATIC_API_HASH;
    ...
    const safeConnect = async (useWSS: boolean): Promise<TelegramClient> => {
      const client = new TelegramClient(stringSession, effectiveApiId, effectiveApiHash, {
        ...commonOptions,
        useWSS,
  ...
  app.post('/api/telegram/auth/send-code', async (req, res) => {
    ...
    const { phone, deliveryType = 'app', apiId = STATIC_API_ID, apiHash = STATIC_API_HASH } = req.body;
    ...
    const numericApiId = Number(apiId) || STATIC_API_ID;
    const stringApiHash = String(apiHash || STATIC_API_HASH);

    console.log(`[MTProto] Official sendCode requested for: ${formattedPhone}, delivery: ${deliveryType}, API_ID: ${numericApiId}`);
    ...
    try {
      const client = await createNewTelegramClient(numericApiId, stringApiHash);
      const isForceSms = deliveryType === 'sms';

      console.log(`[MTProto] Invoking client.sendCode for ${formattedPhone} with forceSMS: ${isForceSms}...`);
      ...
      const isAppDelivery = sendCodeResult.isCodeViaApp !== undefined ? Boolean(sendCodeResult.isCodeViaApp) : !isForceSms;
      const timeout = typeof sendCodeResult.timeout === 'number' ? sendCodeResult.timeout : 60;
      const typeName = isAppDelivery ? 'auth.sentCodeTypeApp' : 'auth.sentCodeTypeSms';

      console.log(`[MTProto] auth.sendCode SUCCESS. phoneCodeHash: ${phoneCodeHash}, isCodeViaApp: ${isAppDelivery}, timeout: ${timeout}`);
      ...
      if (errMsg.includes('API_ID_INVALID')) {
        return res.status(400).json({
          ...
          message: 'مفتاح API_ID أو API_HASH غير صالح. تم استخدام مفاتيح تيليجرام المدمجة (22043994).',
        });
      }
      if (errMsg.includes('PHONE_NUMBER_BANNED')) {
        return res.status(403).json({
          ...
          message: 'هذا الرقم محظور من قبل إدارة تيليجرام.',
        });
      }
      ...
      return res.status(502).json({
        ...
        message: `تعذر إرسال الرمز من خوادم تيليجرام: ${errMsg}. يرجى التحقق من الرقم والاتصال.`,
      });
    }
  });
  ...
  app.post('/api/telegram/auth/resend-code', async (req, res) => {
    ...
    if (sessionData && sessionData.client) {
      try {
        ...
        const resendResult: any = await sessionData.client.invoke(
        ...
        return res.status(400).json({
          success: false,
          message: `تعذر إعادة إرسال الرمز: ${error.message || error.errorMessage || error}`,
        });
      }
    }
  ...
  app.post('/api/telegram/auth/verify-code', async (req, res) => {
    ...
    try {
      ...
      if (password && password.trim()) {
        ...
        try {
          const signInResult: any = await sessionData.client.invoke(
            new Api.auth.SignIn({
              phoneNumber: formattedPhone,
              phoneCodeHash: phoneCodeHash || sessionData.phoneCodeHash,
              phoneCode: cleanCode,
          ...
          if (signMsg.includes('PHONE_CODE_INVALID')) {
            return res.status(400).json({
              success: false,
          ...
          if (signMsg.includes('PHONE_CODE_EXPIRED')) {
            return res.status(400).json({
              success: false,
      ...
      console.log('[MTProto] Real Telegram authentication SUCCESS:', authorizedUser?.id, authorizedUser?.username);
      ...
      // Download user's real avatar if available
      let userAvatar = '';
      try {
        const photoBuf = await sessionData.client.downloadProfilePhoto('me', { isBig: false }).catch(() => null);
        if (photoBuf && Buffer.isBuffer(photoBuf) && photoBuf.length > 0) {
          userAvatar = `data:image/jpeg;base64,${photoBuf.toString('base64')}`;
        }
      } catch (avErr: any) {
        console.warn('[MTProto] Profile photo download notice:', avErr?.message || avErr);
      }

      const meUser = authorizedUser || (await sessionData.client.getMe().catch(() => null)) || {};
      const fullName = [meUser.firstName || meUser.first_name, meUser.lastName || meUser.last_name].filter(Boolean).join(' ') || 'مستخدم تيليجرام';
      ...
      return res.json({
        ...
        user: {
          id: String(meUser.id || Date.now()),
          name: fullName,
          firstName: meUser.firstName || meUser.first_name || 'مستخدم',
          lastName: meUser.lastName || meUser.last_name || '',
          username: meUser.username || '',
          phone: formattedPhone,
          avatar: userAvatar,
          isVerified: Boolean(meUser.verified),
          isPremium: Boolean(meUser.premium),
        },
      ...
      return res.status(400).json({
        success: false,
        error: 'VERIFY_FAILED',
        message: `فشل التحقق من الرمز: ${errMsg}`,
      });
    }
  });
