...
async function startServer() {
  ...
  app.all('/api/telegram/sync', async (req, res) => {
    ...
    // If no active MTProto live client, return unauthorized so client clears fake accounts
    return res.json({
      success: false,
      authenticated: false,
      sessionRevoked: true,
      error: 'NO_ACTIVE_SESSION',
      message: 'لا توجد جلسة نشطة مسجلة مع خوادم تيليجرام. يرجى تسجيل الدخول بحساب حقيقي.',
    });
  });
