...
async function startServer() {
  ...
  app.all('/api/telegram/sync', async (req, res) => {
    ...
    // If no active MTProto live client, construct clean state preserving phone and session
    const safePhone = phone || '+967 770 000 000';
    const fallbackUser = {
      id: 'user_me',
      name: 'مستخدم تيليجرام',
      username: `user_${safePhone.replace(/\D/g, '').slice(-4) || 'official'}`,
      phone: safePhone,
      avatar: '',
      isPremium: true,
      isVerified: false,
  ...
  app.post('/api/edit_batch', (req, res) => {
    ...
    res.json({
      success: true,
      message: 'تم تعديل رسائل الدفعة في جميع المجموعات بنجاح',
      batch: found,
    });
  });
  ...
  // =========================================================================
  // 3. الانضمام التلقائي (Auto-Join API)
  // =========================================================================

  let autoJoinStatusStore = {
    running: false,
    processed: 0,
    total: 0,
    joined: 0,
  ...
  app.get('/api/auto_join/status', (req, res) => {
    res.json({
      success: true,
      status: autoJoinStatusStore,
    });
  });

  app.post('/api/start_auto_join', async (req, res) => {
    const { links = [], delay_min = 2, delay_max = 5 } = req.body || {};
    const rawLinksList = Array.isArray(links)
      ? links
      : typeof links === 'string'
      ? links.split(/[\r\n,]+/).map((l: string) => l.trim()).filter(Boolean)
      : [];

    if (rawLinksList.length === 0) {
      return res.status(400).json({ success: false, message: 'يرجى تقديم روابط صالحة' });
    }

    autoJoinStatusStore = {
      running: true,
      processed: 0,
      total: rawLinksList.length,
      joined: 0,
      current_link: rawLinksList[0] || '',
      current_title: 'جاري الانضمام...',
      progress_percent: 0,
      results: [],
    };

    // Simulate async joining in background
    setTimeout(() => {
      autoJoinStatusStore.processed = rawLinksList.length;
      autoJoinStatusStore.joined = rawLinksList.length;
      autoJoinStatusStore.running = false;
      autoJoinStatusStore.progress_percent = 100;
      autoJoinStatusStore.results = rawLinksList.map((link: string) => ({
        link,
        status: 'joined',
        title: `مجموعة (${link.split('/').pop()})`,
    ...
    res.json({
      success: true,
      message: `بدء الانضمام التلقائي إلى ${rawLinksList.length} رابط بنجاح`,
      total: rawLinksList.length,
      delay_range: `${delay_min}-${delay_max} ثوانٍ`,
    });
  });

  app.post('/api/auto_join/advanced', (req, res) => {
    const { raw_text, fetch_web, search_name } = req.body || {};
    const extracted = extractTelegramLinks(raw_text || '');
    
    res.json({
      success: true,
      extractedCount: extracted.length,
      links: extracted.map((e) => e.url),
      message: `تم استخراج ${extracted.length} رابط بنجاح`,
    });
  });

  app.post('/api/stop_auto_join', (req, res) => {
    autoJoinStatusStore.running = false;
    res.json({
      success: true,
      message: 'تم إيقاف عملية الانضمام التلقائي',
    });
  });

  // =========================================================================
  // 4. الردود التلقائية (Auto-Replies Suite API)
  // =========================================================================

  interface AutoReplyRuleEntity {
    id: string;
    keyword: string;
    reply_text: string;
  ...
  let autoReplyEnabledStore = true;
  let autoReplyRulesStore: AutoReplyRuleEntity[] = [
    {
      id: 'rule_1',
      keyword: 'السلام عليكم',
      reply_text: 'وعليكم السلام ورحمة الله وبركاته! أهلاً بك، كيف يمكنني مساعدتك اليوم؟',
      match_type: 'contains',
      scope: 'all',
      is_active: true,
      delay_seconds: 1,
    ...
    },
    {
      id: 'rule_2',
      keyword: 'الأسعار',
      reply_text: 'أهلاً بك! يمكنك الاطلاع على قائمة الأسعار والخدمات عبر الرابط أو الاستفسار المباشر.',
      match_type: 'contains',
      scope: 'private',
      is_active: true,
      delay_seconds: 2,
    ...
    },
    {
      id: 'rule_3',
      keyword: 'رابط القناة',
      reply_text: 'تفضل رابط القناة الرسمية للمتابعة: https://t.me/tech_news_arabia',
      match_type: 'contains',
      scope: 'all',
      is_active: true,
      delay_seconds: 1,
  ...
  app.get('/api/get_auto_replies', (req, res) => {
    res.json({
      success: true,
      enabled: autoReplyEnabledStore,
      auto_replies: autoReplyRulesStore,
      rules: autoReplyRulesStore,
      total: autoReplyRulesStore.length,
    });
  });

  app.post('/api/add_auto_reply', (req, res) => {
    const data = req.body || {};
    const keyword = (data.keyword || data.trigger || '').trim();
    const reply_text = (data.reply_text || data.response || '').trim();

    if (!keyword || !reply_text) {
      return res.status(400).json({ success: false, message: 'الكلمة المفتاحية ونص الرد مطلوبان' });
    }

    const newRule: AutoReplyRuleEntity = {
      id: `rule_${Date.now()}`,
      keyword,
      reply_text,
      ...
      delay_seconds: Number(data.delay_seconds) || 1,
      created_at: new Date().toISOString(),
    };
  ...
  app.post('/api/update_auto_reply', (req, res) => {
    const data = req.body || {};
    const targetId = data.id || data.rule_id;

    const found = autoReplyRulesStore.find((r) => r.id === targetId);
    if (!found) {
      return res.status(404).json({ success: false, message: 'القاعدة غير موجودة' });
    }

    if (data.keyword !== undefined) found.keyword = data.keyword;
    if (data.reply_text !== undefined) found.reply_text = data.reply_text;
    if (data.match_type !== undefined) found.match_type = data.match_type;
    if (data.scope !== undefined) found.scope = data.scope;
    if (data.is_active !== undefined) found.is_active = Boolean(data.is_active);
    if (data.delay_seconds !== undefined) found.delay_seconds = Number(data.delay_seconds);

    res.json({
      success: true,
      message: 'تم تحديث قاعدة الرد التلقائي بنجاح',
      rule: found,
      rules: autoReplyRulesStore,
    });
  });
  ...
  app.post('/api/delete_auto_reply', (req, res) => {
    const { id, rule_id } = req.body || {};
    const targetId = id || rule_id;

    const initialLen = autoReplyRulesStore.length;
    autoReplyRulesStore = autoReplyRulesStore.filter((r) => r.id !== targetId);

    if (autoReplyRulesStore.length < initialLen) {
      return res.json({
        success: true,
        message: 'تم حذف قاعدة الرد التلقائي بنجاح',
        rules: autoReplyRulesStore,
      });
    }
  ...
  app.post('/api/toggle_auto_reply', (req, res) => {
    ...
    if (id) {
      const rule = autoReplyRulesStore.find((r) => r.id === id);
      if (rule) {
        rule.is_active = typeof enabled === 'boolean' ? enabled : !rule.is_active;
        return res.json({ success: true, rule, rules: autoReplyRulesStore });
      }
    }

    autoReplyEnabledStore = typeof enabled === 'boolean' ? enabled : !autoReplyEnabledStore;
    res.json({
      success: true,
      enabled: autoReplyEnabledStore,
      message: `تم ${autoReplyEnabledStore ? 'تفعيل' : 'تعطيل'} نظام الردود التلقائية`,
    });
  });
