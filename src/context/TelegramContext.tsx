...
interface TelegramContextType {
  ...
  setPipVideoTrack: React.Dispatch<React.SetStateAction<PiPVideoTrack | null>>;
  ...
  setPeekChatData: React.Dispatch<React.SetStateAction<ChatPeekData | null>>;
  fcmDiagnostic: any;
  requestPushPermission: () => Promise<boolean>;
  testSimulateFcmPush: (payload?: any) => void;
...
export const TelegramProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [passcodeSettings, setPasscodeSettings] = useState<PasscodeSettings>(() => {
    try {
      const saved = localStorage.getItem('tg_passcode_settings');
      if (saved) return JSON.parse(saved);
    } catch {}
    return { enabled: false, isLocked: false, timeoutMinutes: 5 };
  });
  const unlockApp = () => {
    setPasscodeSettings((prev) => ({ ...prev, isLocked: false }));
  };
  const [peekChatData, setPeekChatData] = useState<ChatPeekData | null>(null);
  const [fcmDiagnostic, setFcmDiagnostic] = useState<any>({ logs: [], token: null, isSupported: true });
  const requestPushPermission = async () => true;
  const testSimulateFcmPush = (payload?: any) => {
    const newLog = {
      id: Date.now().toString(),
      title: payload?.title || 'FCM Test Push',
      body: payload?.body || 'Test push notification received',
      timestamp: new Date().toLocaleTimeString(),
      status: 'received',
      payload: payload || {},
    };
    setFcmDiagnostic((prev: any) => ({
      ...prev,
      logs: [newLog, ...(prev?.logs || [])],
    }));
  };
  const clearFcmDiagnosticHistory = () => setFcmDiagnostic((prev: any) => ({ ...prev, logs: [] }));
  ...
  const login = (data: { name: string; phone: string; username?: string; avatar?: string; bio?: string; sessionString?: string }) => {
    ...
    const personalizedSavedMsg: Chat = {
      ...
      unreadCount: 0,
      isPinned: true,
      description: 'مساحتك السحابية الخاصة لحفظ الرسائل، الصور، الفيديوهات، الملفات والملاحظات مع مزامنة فورية على جميع أجهزتك.',
      lastMessage: {
        id: `m_s_1_${Date.now()}`,
        senderName: 'You',
    ...
    const initialAccChats: Chat[] = [
      personalizedSavedMsg,
      ...INITIAL_CHATS.filter((c) => c.id !== 'chat_saved_messages'),
    ];
