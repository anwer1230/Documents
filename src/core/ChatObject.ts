...
export class ChatObject {
  ...
  /**
   * Check if the authenticated user is NOT a member of the group/channel
   * Replicates ChatObject.isNotInChat(TLRPC.Chat) from Telegram Android DrKLO.
   */
  public static isNotInChat(chat?: TelegramChatEntity | TLRPC.Chat | null): boolean {
    if (!chat) return false;
    const c = chat as any;
    if (c.type === 'private' || c.type === 'saved' || c.type === 'bot') {
      return false;
    }
    return Boolean(c.left === true || c.isMember === false || c.kicked === true);
  }
}
