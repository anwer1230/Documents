...
export class MessagesStorage {
  ...
  /**
   * DrKLO MessagesStorage.putMessage
   * Persists a single message object
   */
  public putMessage(param1: any, param2?: any): void {
    if (typeof param1 === 'string') {
      const dialogId = param1;
      const message = param2;
      if (message) this.putMessages([message], dialogId);
    } else {
      const message = param1;
      const dialogId = param2 || String(message.chatId || message.chat_id || '0');
      if (message) this.putMessages([message], dialogId);
    }
  }
  ...
  /**
   * DrKLO MessagesStorage.putDialogs
   * Persists dialog batch
   */
