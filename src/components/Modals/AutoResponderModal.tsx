...
export const AutoResponderModal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Fetch initial from backend if available
    fetch('/api/get_auto_replies')
      .then((r) => r.json())
      .then((data) => {
        if (data && Array.isArray(data.rules) && data.rules.length > 0) {
          if (backgroundSyncService.getAutoReplyRules().length === 0) {
            data.rules.forEach((r: any) => {
              backgroundSyncService.addAutoReplyRule({
                ...
                scope: r.scope || 'all',
                isEnabled: r.is_active !== undefined ? r.is_active : true,
              });
            });
          }
        }
                ...
                >
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => {
                          backgroundSyncService.toggleRule(rule.id);
                          fetch('/api/toggle_auto_reply', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                      ...
                      <button
                        onClick={() => {
                          backgroundSyncService.deleteRule(rule.id);
