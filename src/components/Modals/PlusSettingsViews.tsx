...
// BroadcastChannel for instant cross-tab / cross-device synchronization
const syncChannel = typeof window !== 'undefined' && 'BroadcastChannel' in window ? new BroadcastChannel('tg_plus_settings_sync') : null;
...
export const usePlusConfig = () => {
  ...
  // Hydrate from Dexie IndexedDB if available
  useEffect(() => {
    let isMounted = true;
    (async () => {
      try {
        const record = await telegramDb.plusSettings.get('global');
        if (record && record.config && isMounted) {
          setConfigState((prev) => {
            const merged = { ...prev, ...record.config };
            try {
              localStorage.setItem('tg_plus_config_v2', JSON.stringify(merged));
            } catch {}
            return merged;
          });
        }
      } catch (err) {
        console.warn('Could not read plus config from Dexie DB', err);
      }
    })();

    // Listen to cross-tab BroadcastChannel
    const handleBroadcast = (evt: MessageEvent) => {
      if (evt.data && evt.data.type === 'PLUS_CONFIG_UPDATED' && evt.data.config) {
    ...
    const handleLocalEvent = (evt: Event) => {
      const customEvt = evt as CustomEvent<PlusConfig>;
      if (customEvt.detail) {
        setConfigState(customEvt.detail);
      }
    };

    // Listen to storage event (for cross-window)
    const handleStorage = (evt: StorageEvent) => {
      if (evt.key === 'tg_plus_config_v2' && evt.newValue) {
        try {
          setConfigState(JSON.parse(evt.newValue));
        } catch {}
      }
    };
    ...
    window.addEventListener('tg_plus_config_updated', handleLocalEvent);
    ...
    return () => {
      ...
      window.removeEventListener('tg_plus_config_updated', handleLocalEvent);
      window.removeEventListener('storage', handleStorage);
    };
  }, []);

  const saveAndSync = (nextConfig: PlusConfig) => {
    try {
      localStorage.setItem('tg_plus_config_v2', JSON.stringify(nextConfig));
    } catch (e) {
      console.error('Failed to save plus config to localStorage', e);
    }

    // Persist to Dexie IndexedDB
    telegramDb.plusSettings
      .put({ id: 'global', config: nextConfig, updatedAt: Date.now() })
      .catch((e) => console.warn('Failed to save plus config to Dexie:', e));

    // Broadcast cross-tab & cross-window
    if (syncChannel) {
      try {
        syncChannel.postMessage({ type: 'PLUS_CONFIG_UPDATED', config: nextConfig, timestamp: Date.now() });
      } catch {}
    }

    // Dispatch local event for same-window components
    try {
      window.dispatchEvent(new CustomEvent('tg_plus_config_updated', { detail: nextConfig }));
    } catch {}
  };
  ...
  const setConfig = (next: PlusConfig | ((prev: PlusConfig) => PlusConfig)) => {
    setConfigState((prev) => {
      const resolved = typeof next === 'function' ? (next as any)(prev) : next;
      saveAndSync(resolved);
      return resolved;
    });
  };
  ...
  const resetConfig = () => {
    ...
    saveAndSync(DEFAULT_PLUS_CONFIG);
  };
