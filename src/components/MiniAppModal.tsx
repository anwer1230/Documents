...
import { X, MoreVertical, RotateCcw, ExternalLink, ShieldCheck, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
        ...
        <div className={`flex items-center justify-between px-4 py-3 border-b select-none shrink-0 ${isDark ? 'bg-[#242f3d] border-gray-700/60' : 'bg-gray-50 border-gray-200'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400">
                {isAr ? 'تطبيق ويب مصغر لتيليجرام' : 'Telegram Mini App Webview'}
              </p>
            </div>
          </div>
        ...
        <div className={`px-4 py-2 border-b flex items-center gap-2 overflow-x-auto text-xs shrink-0 ${isDark ? 'bg-[#1c2733] border-gray-800' : 'bg-gray-100 border-gray-200'}`}>
        ...
        <form onSubmit={handleLoadCustomUrl} className={`px-4 py-2 border-b flex items-center gap-2 shrink-0 ${isDark ? 'bg-[#18222d] border-gray-800' : 'bg-gray-50 border-gray-200'}`}>
          <input
            type="text"
            value={customUrl}
            onChange={(e) => setCustomUrl(e.target.value)}
            placeholder={isAr ? 'أدخل رابط تطبيق ويب مصغر (https://...)' : 'Enter Mini App URL (https://...)'}
            className={`flex-1 px-3 py-1.5 rounded-lg text-xs outline-none border ${
              isDark ? 'bg-[#242f3d] border-gray-700 text-white placeholder-gray-400' : 'bg-white border-gray-300 text-gray-900'
            }`}
          />
          <button
            type="submit"
