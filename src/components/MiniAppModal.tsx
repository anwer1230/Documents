...
export const MiniAppModal: React.FC<MiniAppModalProps> = ({
  ...
  // Telegram WebApp postMessage Bridge (matches official Telegram Web K/A SDK)
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      try {
        ...
        const sendReply = (replyType: string, replyData: any) => {
          if (iframeRef.current && iframeRef.current.contentWindow) {
            iframeRef.current.contentWindow.postMessage(
              JSON.stringify({ eventType: replyType, eventData: replyData }),
              '*'
            );
          }
        };
        ...
        switch (data.eventType) {
          ...
          case 'web_app_trigger_haptic_feedback':
            if (data.eventData?.type && 'vibrate' in navigator) {
              try {
                if (data.eventData.type === 'impact') {
                  navigator.vibrate(15);
                } else if (data.eventData.type === 'notification') {
          ...
          case 'web_app_request_theme':
            sendReply('theme_changed', {
              theme_params: {
                bg_color: isDark ? '#17212b' : '#ffffff',
                text_color: isDark ? '#ffffff' : '#000000',
                hint_color: isDark ? '#708499' : '#999999',
                link_color: '#3390ec',
                button_color: '#3390ec',
                button_text_color: '#ffffff',
                secondary_bg_color: isDark ? '#242f3d' : '#f4f4f5',
              },
            });
            break;

          case 'web_app_request_viewport':
            sendReply('viewport_changed', {
              height: 700,
          ...
          case 'web_app_storage_get':
            if (data.eventData?.key) {
              const val = localStorage.getItem(`twa_${botUsername}_${data.eventData.key}`);
              sendReply('storage_get_result', { key: data.eventData.key, value: val });
            }
            break;

          case 'web_app_storage_set':
            if (data.eventData?.key && data.eventData?.value !== undefined) {
              localStorage.setItem(`twa_${botUsername}_${data.eventData.key}`, String(data.eventData.value));
            }
            break;
  ...
  }, [onClose, onSendData, isDark, botUsername]);
